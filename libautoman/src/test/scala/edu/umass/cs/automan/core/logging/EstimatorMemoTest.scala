...
import edu.umass.cs.automan.core.answer.{Estimate, Answer}

class EstimatorMemoTest extends FlatSpec with Matchers {

  "An estimator program" should "correctly recall answers at no cost" in {
    val confidence = 0.95
    val confidence_interval = SymmetricCI(50)
    // from R using rnorm(500, mean = 633, sd = 100)
    // mean = 623.87
    // n = 500
    // sd = 99.1885
    val mock_answers = makeMocks[Double](630,656,598,586,607,629,715,632,645,670,
    ...
    automan(mt, test_mode = true, in_mem_db = true) {
      countJellies().answer match {
        case Estimate(est, low, high, cost, conf, _, _) =>
          ...
          (cost >= BigDecimal(0.48)) should be (true)   // NOTE that we spend here
        case _ =>
          fail()
      }
      ...
      countJellies2().answer match {
        case Estimate(est, low, high, cost, conf, _, _) =>
          ...
          (est > low && est < high) should be (true)
          (conf >= confidence) should be (true)
          (cost == BigDecimal(0)) should be (true)      // NOTE that we save here
        case _ =>
          fail()
      }
    }
