...
class MTHugoQuestion extends HugoQuestion with MTurkQuestion {
  type QuestionOptionType = MTQuestionOption
  override type A = HugoQuestion#A
  ...
  override def toMockResponse(question_id: UUID, response_time: Date, a: A, worker_id: UUID) : HugoMockResponse = {
    HugoMockResponse(question_id, response_time, a, worker_id)
  }
  override protected[mturk] def fromXML(x: scala.xml.Node) : A = { // HugoQuestion#A
    ...
    val answers = x \\ "Answer"
    println(answers)

    //(x \\ "Answer" \\ "SelectionIdentifier").map{si => Symbol(si.text.drop(1))}.toSet
    (Set(Symbol("Oscar the Grouch")), Set(Symbol("Oscar the Grouch")))
  }
