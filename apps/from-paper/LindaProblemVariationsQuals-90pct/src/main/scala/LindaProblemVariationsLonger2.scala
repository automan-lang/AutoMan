...
object LindaProblemVariationsLonger2 extends App {
  val opts = Utilities.unsafe_optparse(args, "LindaProblemVariationsLonger2")
  ...
  def conjunction_fallacy(): DSL.SurveyOutcome[List[Any]] = SurveyWithQuals(
    ...
    questions = List(
      ...
      radioQuestion(
        text = "${description}<br><br>Which is more probable?",
        options = (
